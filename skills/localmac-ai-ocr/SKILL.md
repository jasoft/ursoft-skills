...
优先使用当前工作区里的 `ocr` 和 `gui`。先在当前目录确认入口：

```bash
pwd
ls
test -x ./ocr && test -x ./gui
```

如果命令不在当前目录，先用 `command -v`、`pwd`、`ls` 或 `rg --files` 找到实际路径，再继续。不要在 skill、提交内容或对外说明里暴露用户本地目录名。
...
```bash
AISTUDIO_OCR_TOKEN="$AISTUDIO_OCR_TOKEN" ./ocr doctor
./gui doctor --json
```
...
```bash
AISTUDIO_OCR_TOKEN="$AISTUDIO_OCR_TOKEN" ./ocr ocr /tmp/rdp.png --format json
AISTUDIO_OCR_TOKEN="$AISTUDIO_OCR_TOKEN" ./ocr find /tmp/rdp.png --query 腾讯控股 --mode contains --center --format json
AISTUDIO_OCR_TOKEN="$AISTUDIO_OCR_TOKEN" ./ocr click-text /tmp/rdp.png --query 腾讯控股 --mode contains
```

只需要快速看文字时，可用：

```bash
AISTUDIO_OCR_TOKEN="$AISTUDIO_OCR_TOKEN" ./gui ocr /tmp/rdp.png --format text
```
...
- 只看纯文本：
```bash
AISTUDIO_OCR_TOKEN="$AISTUDIO_OCR_TOKEN" ./ocr ocr /tmp/rdp.png --format text
```

- 查文字并输出中心点：
```bash
AISTUDIO_OCR_TOKEN="$AISTUDIO_OCR_TOKEN" ./ocr find /tmp/rdp.png --query 00700 --mode exact --center --format json
```

- 对识别结果打框：
```bash
AISTUDIO_OCR_TOKEN="$AISTUDIO_OCR_TOKEN" ./ocr annotate /tmp/rdp.png /tmp/rdp-annotated.png --query 腾讯控股
```
