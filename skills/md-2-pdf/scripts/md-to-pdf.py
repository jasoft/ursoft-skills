...
def contains_cjk(text: str) -> bool:
    """Return True when the content contains common CJK ideographs."""
    return any('\u4e00' <= char <= '\u9fff' for char in text)


def register_cjk_font(verbose: bool = False) -> str:
    """Register a CJK-capable font when available and return its name."""
    for font_name, font_path in FONT_CANDIDATES:
        ...
        try:
            ...
            if verbose:
                print(f"Using CJK font: {font_name} ({font_path})")
            return font_name
            ...
            continue
    if verbose:
        print("  Warning: No CJK font registered, falling back to Helvetica")
    return 'Helvetica'
...
def create_styles(base_font: str) -> dict:
...
def add_page_number(canvas, doc, page_font: str):
    """Add page numbers to the footer."""
    page_num = canvas.getPageNumber()
    text = f"Page {page_num}"
    canvas.saveState()
    canvas.setFont(page_font, 9)
...
def create_pdf(markdown_file: str, output_pdf: Optional[str] = None, verbose: bool = False):
    ...
    base_font = register_cjk_font(verbose) if contains_cjk(content) else 'Helvetica'
    ...
    styles = create_styles(base_font)
    story = []
    ...
    try:
        doc.build(
            story,
            onFirstPage=lambda canvas, doc: add_page_number(canvas, doc, base_font),
            onLaterPages=lambda canvas, doc: add_page_number(canvas, doc, base_font),
        )
